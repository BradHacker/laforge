...
type EnvironmentEdges struct {
	...
	namedValidations      map[string][]*Validation
}
...
func (e *Environment) QueryValidations() *ValidationQuery {
	return NewEnvironmentClient(e.config).QueryValidations(e)
}
...
// NamedValidations returns the Validations named value or an error if the edge was not
// loaded in eager-loading with this name.
func (e *Environment) NamedValidations(name string) ([]*Validation, error) {
	if e.Edges.namedValidations == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := e.Edges.namedValidations[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (e *Environment) appendNamedValidations(name string, edges ...*Validation) {
	if e.Edges.namedValidations == nil {
		e.Edges.namedValidations = make(map[string][]*Validation)
	}
	if len(edges) == 0 {
		e.Edges.namedValidations[name] = []*Validation{}
	} else {
		e.Edges.namedValidations[name] = append(e.Edges.namedValidations[name], edges...)
	}
}
