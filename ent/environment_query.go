...
type EnvironmentQuery struct {
	...
	withNamedValidations      map[string]*ValidationQuery
...
func (eq *EnvironmentQuery) QueryValidations() *ValidationQuery {
	query := (&ValidationClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EnvironmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Environment, error) {
	...
	for name, query := range eq.withNamedValidations {
		if err := eq.loadValidations(ctx, query, nodes,
			func(n *Environment) { n.appendNamedValidations(name) },
			func(n *Environment, e *Validation) { n.appendNamedValidations(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (eq *EnvironmentQuery) loadValidations(ctx context.Context, query *ValidationQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *Validation)) error {
	...
	query.Where(predicate.Validation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(environment.ValidationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "environment_validations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedValidations tells the query-builder to eager-load the nodes that are connected to the "Validations"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (eq *EnvironmentQuery) WithNamedValidations(name string, opts ...func(*ValidationQuery)) *EnvironmentQuery {
	query := (&ValidationClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if eq.withNamedValidations == nil {
		eq.withNamedValidations = make(map[string]*ValidationQuery)
	}
	eq.withNamedValidations[name] = query
	return eq
}
