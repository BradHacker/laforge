	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validation":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&ValidationClient{config: at.config}).Query()
			)
			if err := query.collectField(ctx, opCtx, field, path, satisfies...); err != nil {
				return err
			}
			at.withValidation = query
		case "command":
...
func (a *AnsibleQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			if _, ok := fieldSeen[ansible.FieldValidations]; !ok {
				selectedFields = append(selectedFields, ansible.FieldValidations)
				fieldSeen[ansible.FieldValidations] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (c *CommandQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			if _, ok := fieldSeen[command.FieldValidations]; !ok {
				selectedFields = append(selectedFields, command.FieldValidations)
				fieldSeen[command.FieldValidations] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (dr *DNSRecordQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			if _, ok := fieldSeen[dnsrecord.FieldValidations]; !ok {
				selectedFields = append(selectedFields, dnsrecord.FieldValidations)
				fieldSeen[dnsrecord.FieldValidations] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&ValidationClient{config: e.config}).Query()
			)
			if err := query.collectField(ctx, opCtx, field, path, satisfies...); err != nil {
				return err
			}
			e.WithNamedValidations(alias, func(wq *ValidationQuery) {
				*wq = *query
			})
		case "hclID":
...
func (fd *FileDeleteQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			if _, ok := fieldSeen[filedelete.FieldValidations]; !ok {
				selectedFields = append(selectedFields, filedelete.FieldValidations)
				fieldSeen[filedelete.FieldValidations] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (fd *FileDownloadQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			if _, ok := fieldSeen[filedownload.FieldValidations]; !ok {
				selectedFields = append(selectedFields, filedownload.FieldValidations)
				fieldSeen[filedownload.FieldValidations] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (fe *FileExtractQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			if _, ok := fieldSeen[fileextract.FieldValidations]; !ok {
				selectedFields = append(selectedFields, fileextract.FieldValidations)
				fieldSeen[fileextract.FieldValidations] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (s *ScriptQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "validations":
			if _, ok := fieldSeen[script.FieldValidations]; !ok {
				selectedFields = append(selectedFields, script.FieldValidations)
				fieldSeen[script.FieldValidations] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
// CollectFields tells the query-builder to eagerly load connected nodes by resolver context.
func (v *ValidationQuery) CollectFields(ctx context.Context, satisfies ...string) (*ValidationQuery, error) {
	fc := graphql.GetFieldContext(ctx)
	if fc == nil {
		return v, nil
	}
...
func (v *ValidationQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	var (
		unknownSeen    bool
		fieldSeen      = make(map[string]struct{}, len(validation.Columns))
		selectedFields = []string{validation.FieldID}
	)
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "users":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&UserClient{config: v.config}).Query()
			)
			if err := query.collectField(ctx, opCtx, field, path, satisfies...); err != nil {
				return err
			}
			v.WithNamedUsers(alias, func(wq *UserQuery) {
				*wq = *query
			})
		case "environment":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&EnvironmentClient{config: v.config}).Query()
			)
			if err := query.collectField(ctx, opCtx, field, path, satisfies...); err != nil {
				return err
			}
			v.withEnvironment = query
		case "hclID":
			if _, ok := fieldSeen[validation.FieldHCLID]; !ok {
				selectedFields = append(selectedFields, validation.FieldHCLID)
				fieldSeen[validation.FieldHCLID] = struct{}{}
			}
		case "validationType":
			if _, ok := fieldSeen[validation.FieldValidationType]; !ok {
				selectedFields = append(selectedFields, validation.FieldValidationType)
				fieldSeen[validation.FieldValidationType] = struct{}{}
			}
		case "hash":
			if _, ok := fieldSeen[validation.FieldHash]; !ok {
				selectedFields = append(selectedFields, validation.FieldHash)
				fieldSeen[validation.FieldHash] = struct{}{}
			}
		case "regex":
			if _, ok := fieldSeen[validation.FieldRegex]; !ok {
				selectedFields = append(selectedFields, validation.FieldRegex)
				fieldSeen[validation.FieldRegex] = struct{}{}
			}
		case "ip":
			if _, ok := fieldSeen[validation.FieldIP]; !ok {
				selectedFields = append(selectedFields, validation.FieldIP)
				fieldSeen[validation.FieldIP] = struct{}{}
			}
		case "url":
			if _, ok := fieldSeen[validation.FieldURL]; !ok {
				selectedFields = append(selectedFields, validation.FieldURL)
				fieldSeen[validation.FieldURL] = struct{}{}
			}
		case "port":
			if _, ok := fieldSeen[validation.FieldPort]; !ok {
				selectedFields = append(selectedFields, validation.FieldPort)
				fieldSeen[validation.FieldPort] = struct{}{}
			}
		case "hostname":
			if _, ok := fieldSeen[validation.FieldHostname]; !ok {
				selectedFields = append(selectedFields, validation.FieldHostname)
				fieldSeen[validation.FieldHostname] = struct{}{}
			}
		case "nameservers":
			if _, ok := fieldSeen[validation.FieldNameservers]; !ok {
				selectedFields = append(selectedFields, validation.FieldNameservers)
				fieldSeen[validation.FieldNameservers] = struct{}{}
			}
		case "packageName":
			if _, ok := fieldSeen[validation.FieldPackageName]; !ok {
				selectedFields = append(selectedFields, validation.FieldPackageName)
				fieldSeen[validation.FieldPackageName] = struct{}{}
			}
		case "username":
			if _, ok := fieldSeen[validation.FieldUsername]; !ok {
				selectedFields = append(selectedFields, validation.FieldUsername)
				fieldSeen[validation.FieldUsername] = struct{}{}
			}
		case "groupName":
			if _, ok := fieldSeen[validation.FieldGroupName]; !ok {
				selectedFields = append(selectedFields, validation.FieldGroupName)
				fieldSeen[validation.FieldGroupName] = struct{}{}
			}
		case "filePath":
			if _, ok := fieldSeen[validation.FieldFilePath]; !ok {
				selectedFields = append(selectedFields, validation.FieldFilePath)
				fieldSeen[validation.FieldFilePath] = struct{}{}
			}
		case "searchString":
			if _, ok := fieldSeen[validation.FieldSearchString]; !ok {
				selectedFields = append(selectedFields, validation.FieldSearchString)
				fieldSeen[validation.FieldSearchString] = struct{}{}
			}
		case "serviceName":
			if _, ok := fieldSeen[validation.FieldServiceName]; !ok {
				selectedFields = append(selectedFields, validation.FieldServiceName)
				fieldSeen[validation.FieldServiceName] = struct{}{}
			}
		case "filePermission":
			if _, ok := fieldSeen[validation.FieldFilePermission]; !ok {
				selectedFields = append(selectedFields, validation.FieldFilePermission)
				fieldSeen[validation.FieldFilePermission] = struct{}{}
			}
		case "serviceStatus":
			if _, ok := fieldSeen[validation.FieldServiceStatus]; !ok {
				selectedFields = append(selectedFields, validation.FieldServiceStatus)
				fieldSeen[validation.FieldServiceStatus] = struct{}{}
			}
		case "processName":
			if _, ok := fieldSeen[validation.FieldProcessName]; !ok {
				selectedFields = append(selectedFields, validation.FieldProcessName)
				fieldSeen[validation.FieldProcessName] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
type validationPaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []ValidationPaginateOption
}

func newValidationPaginateArgs(rv map[string]any) *validationPaginateArgs {
	args := &validationPaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
