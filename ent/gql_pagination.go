...
func (c *ValidationConnection) build(nodes []*Validation, pager *validationPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	c.Edges = make([]*ValidationEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ValidationEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
func newValidationPager(opts []ValidationPaginateOption, reverse bool) (*validationPager, error) {
	pager := &validationPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
...
func (p *validationPager) applyCursors(query *ValidationQuery, after, before *Cursor) (*ValidationQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultValidationOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *validationPager) applyOrder(query *ValidationQuery) *ValidationQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	if p.order.Field != DefaultValidationOrder.Field {
		query = query.Order(DefaultValidationOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *validationPager) orderExpr(query *ValidationQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultValidationOrder.Field {
			b.Comma().Ident(DefaultValidationOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}
...
func (v *ValidationQuery) Paginate(
	...
	pager, err := newValidationPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
...
type ValidationOrderField struct {
	// Value extracts the ordering value from the given Validation.
	Value    func(*Validation) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) validation.OrderOption
...
var DefaultValidationOrder = &ValidationOrder{
	...
	Field: &ValidationOrderField{
		Value: func(v *Validation) (ent.Value, error) {
			return v.ID, nil
		},
		column: validation.FieldID,
		toTerm: validation.ByID,
