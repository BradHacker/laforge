...
func (r *mutationResolver) CreateAgentTasks(ctx context.Context, hostHclid string, command model.AgentCommand, buildUUID string, args []string, teams []int) ([]*ent.AgentTask, error) {
	...
	for _, team_number := range teams {
		...
		entProvisionedHost, err := entTeam.QueryProvisionedNetworks().QueryProvisionedHosts().Where(provisionedhost.HasHostWith(host.HCLID(hostHclid))).All(ctx)
		if err != nil {
...
// NewState is the resolver for the newState field.
...
// EnvironmentFilepath is the resolver for the environmentFilepath field.
...
// StatusFor is the resolver for the statusFor field.
